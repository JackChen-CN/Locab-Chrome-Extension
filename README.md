...
### 一分钟上手
1. **安装扩展**：加载到Chrome（见下方安装方法）
2. **标记单词**：在网页上选中单词 → 右键 → "标记生词"
3. **管理单词**：点击工具栏图标打开管理面板
4. **复习单词**：切换到"复习模式"标签页
5. **定位单词**：点击"定位"按钮返回原文
...
让我们通过一个完整的例子来了解如何使用 Locab：

**场景**：您在阅读英文技术文章时遇到不认识的单词 "algorithm"

#### 第一步：标记单词
1. 在网页上选中单词 "algorithm"
2. **右键点击**，选择"标记生词"
3. 扩展自动：
   - 获取中文翻译"算法"
   - 提取完整句子："The algorithm efficiently sorts the data."
   - 计算位置：第3行，第2个单词
   - 记录页面URL和XPath
4. 页面右上角显示"已标记单词: algorithm"

#### 第二步：管理单词库
1. 点击浏览器工具栏中的 **Locab 图标** (蓝色书本图标)
2. 弹出窗口显示所有标记的单词
3. 在"单词列表"标签页中，您可以看到：
   - 单词："algorithm"
   - 翻译："算法"
   - 句子预览："The algorithm efficiently sorts the data."
   - 来源网址：example.com
   - 记录时间：刚刚
   - 位置：行3，词2

#### 第三步：搜索和过滤
- 在搜索框中输入"alg"可以快速找到"algorithm"
- 也可以搜索中文"算"来查找相关翻译
- 搜索结果实时更新，方便复习特定主题的单词
...
#### 第五步：定位到原文
1. 在单词列表或复习卡片中点击"**定位**"按钮
2. 扩展自动：
   - 如果仍在原页面：**滚动到单词位置**，黄色高亮显示3秒
   - 如果不在原页面：**新标签页打开原网页**，加载后自动定位
3. 您可以：
   - 重新阅读上下文，加深理解
   - 查看单词在原文中的实际用法
   - 复习相关概念
...
#### 第六步：数据管理
- **导出数据**：点击"导出"按钮，获得`vocab_export_20260118.txt`
- **删除单词**：点击删除按钮移除不再需要的记录
- **清空数据**：谨慎使用，可清空所有记录
...
| 功能 | 说明 | 使用场景 |
|------|------|----------|
| **搜索** | 按单词/翻译/句子搜索 | 复习特定主题单词 |
| **排序** | 按时间倒序排列 | 查看最新标记的单词 |
| **预览** | 显示句子前100字符 | 快速回忆上下文 |
| **定位** | 一键返回原文 | 重新学习上下文 |
| **删除** | 移除单个记录 | 清理已掌握的单词 |
...
#### 🎯 定位功能详解
**定位策略优先级**：
1. **精确定位**：使用XPath + 行号/词序号（最准确）
2. **降级定位**：如果XPath失效，使用行号/词序号
3. **全文搜索**：如果位置信息失效，在页面中搜索单词

**跨页面处理**：
- **同页面**：直接滚动并高亮
- **不同页面**：新标签页打开 → 等待加载 → 自动定位
- **页面变化**：如果原页面内容更新，降级使用搜索功能

#### 💾 数据持久化
- **存储位置**：浏览器本地存储 (`chrome.storage.local`)
- **数据格式**：JSON数组，每条记录包含完整信息
- **导出格式**：JSONL（每行一个JSON对象）
- **导入功能**：暂无，但JSONL格式易于处理
...
algorithm
算法
"The algorithm efficiently sorts..."
来源: example.com · 刚刚 · 位置: 行3, 词2
[定位] [删除]
──────────────────────
```
...
#### 高效学习技巧
1. **立即标记**：遇到生词立即标记，保持阅读流畅
2. **定期复习**：每天花5分钟复习标记的单词
3. **上下文学习**：使用定位功能回到原文，理解单词用法
4. **主题分类**：用搜索功能复习同一主题的单词

#### 数据管理技巧
1. **定期导出**：每周导出数据作为备份
2. **清理已掌握**：定期删除已掌握的单词，保持列表简洁
3. **专注领域**：标记专业领域单词，建立专属词汇库

#### 浏览器集成技巧
1. **固定到工具栏**：右键点击Locab图标，选择"固定到工具栏"
2. **快捷键**：暂无，但可通过Chrome扩展管理设置
3. **多窗口同步**：数据在Chrome实例间同步

### ❓ 常见问题 (FAQ)

#### Q: 为什么选中单词后右键菜单没有"标记生词"？
**A**: 可能原因：
1. 选择了多个单词或包含空格
2. 扩展未正确加载，请检查`chrome://extensions/`
3. 需要刷新页面使内容脚本生效
...
#### Q: 数据会同步到其他设备吗？
**A**: 如果启用Chrome同步，`chrome.storage.local`数据会同步

#### Q: 最多可以存储多少单词？
**A**: Chrome存储限制约5MB，约可存储数千条记录

#### Q: 如何批量删除单词？
**A**: 目前只能逐个删除，可使用"清空"功能删除全部
...
目前Chrome扩展仅支持桌面版Chrome浏览器，不支持移动设备。但导出的JSONL文件可在任何设备上查看和处理。
